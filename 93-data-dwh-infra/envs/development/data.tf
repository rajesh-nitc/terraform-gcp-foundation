...
data "google_projects" "dwh_projects" {
  filter = "parent.id:${split("/", var.folder_id)[1]} name:*${var.project_suffix}* labels.application_name=${var.business_code}-dwh labels.environment=${var.environment} lifecycleState=ACTIVE"
}
