...
data "google_projects" "landing_projects" {
  filter = "parent.id:${split("/", local.folder_id)[1]} name:*landing* labels.application_name=${var.business_code}-landing labels.environment=${var.environment} lifecycleState=ACTIVE"
}
